...
window.fetch = async (url, options = {}) => {
  ...
  if (url.endsWith('/api/ads/active-playlist')) return json({ ok: true, ads: playlist });
  ...
  if (url.endsWith('/confirm-booking')) return json({ ok: true, pending: { requestId: 'AD-REQUEST' } });
...
async function run() {
  ...
  assert(document.querySelector('svg[aria-label="Secure Reliv advertisement payment QR code"]'), 'kiosk renders a backend HTTPS payment QR');
  let spoken = 0;
  const onSpoken = () => { spoken++; };
  window.addEventListener('reliv_spoken_text', onSpoken);
  await act(async () => { speech.speakText('Delayed page prompt'); speech.speak('leaderboard'); });
  assert(spoken === 0, 'ad payment overlay blocks delayed page speech');
  window.removeEventListener('reliv_spoken_text', onSpoken);
  await click('Back to health');
  await tick(2500);
  assert(!document.querySelector('.kiosk-payment-active-screen'), 'dismissed payment does not trap health users on every poll');
  ...
  assert(sent.code === '0042' && sent.requestId === 'AD-REQUEST', 'activation preserves leading zero and payment binding');
  ...
  await mount(<KioskAdPlayer />);
  await tick(20000); await tick(5000);
