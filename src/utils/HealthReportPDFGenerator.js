...
class HealthReportPDFGenerator {
  ...
  drawVitals(vitals, computed) {
    ...
    const vitalsData = [
      ...
      },
      {
        ...
        status: computed.bpm.label,
        note: computed.bpm.advice
      },
      {
    ...
    const cardMargin = 20;
    const cardWidth = (this.pageWidth - 2 * this.margin - cardMargin) / 2;
  ...
  generateSummaryPoints(vitals, computed, bodyComposition) {
    const points = [];
    ...
    // Oxygen Summary
    points.push(`Oxygen Saturation: ${vitals.oxygen || '—'}% - ${computed.oxygen.label}. ${computed.oxygen.advice}`);

    // Heart Rate Summary
    points.push(`Heart Rate: ${vitals.bpm || '—'} BPM - ${computed.bpm.label}. ${computed.bpm.advice}`);

    // Temperature Summary
    points.push(`Body Temperature: ${vitals.temperature || '—'} °F - ${computed.temp.label}. ${computed.temp.advice}`);
    if (Number.isFinite(bodyComposition?.bmi)) {
  ...
  generateReport(healthData, bodyCompositionData, ecoStats) {
    return new Promise((resolve) => {
      ...
      this.doc.on('end', () => {
        const total = buffers.reduce((size, chunk) => size + chunk.length, 0);
      ...
      const computed = {
        ...
        oxygen: this.assessSpO2(vitals.oxygen),
        bpm: this.assessPulse(vitals.bpm),
        temp: this.assessTempF(vitals.temperature),
