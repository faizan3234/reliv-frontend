...
export default function KioskGuardian() {
  ...
  useEffect(() => {
    ...
    if (currentPath.startsWith('/mobile-entry') || currentPath === '/h' || currentPath.startsWith('/admin-x7k9')) {
    ...
    const handleKeyDown = (e) => {
      ...
      // Always allow standard edit shortcuts inside editable fields
      if ((e.ctrlKey || e.metaKey) && isEditable && ['a', 'c', 'v', 'x', 'z', 'y'].includes(key)) {
        return;
      }
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (['=', '-', '0', '+'].includes(key)) {
          e.preventDefault();
          e.stopPropagation();
          console.log('[KioskGuardian] ⛔ Zoom blocked');
          return false;
        }
      ...
      if (e.altKey) {
        const blockedAltKeys = ['tab', 'f4', 'd', 'home', 'left', 'right', 'space'];
