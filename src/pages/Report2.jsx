...
function assessBodyFat(vitals, patient, history) {
  ...
  // Status based on gender — ACE/WHO categories
  let status = null;
  if (sex === 1) { // Male
    if (bodyFat < 6) status = "Essential Fat";
    else if (bodyFat >= 6 && bodyFat < 14) status = "Athletic";
    else if (bodyFat >= 14 && bodyFat < 18) status = "Fitness";
    else if (bodyFat >= 18 && bodyFat < 25) status = "Normal";
    else status = "High";
  } else { // Female
    if (bodyFat < 14) status = "Essential Fat";
    else if (bodyFat >= 14 && bodyFat < 21) status = "Athletic";
    else if (bodyFat >= 21 && bodyFat < 25) status = "Fitness";
    else if (bodyFat >= 25 && bodyFat < 32) status = "Normal";
    else status = "High";
  }
...
function assessMuscle(vitals, patient, history) {
  ...
  // Status based on gender — calibrated to formula output range
  // Male formula produces 33-46%, Female produces 24-38%
  let status = null;
  if (sex === 1) { // Male
    if (musclePercent >= 44) status = "Excellent";
    else if (musclePercent >= 36) status = "Normal";
    else status = "Below Average";
  } else { // Female
    if (musclePercent >= 36) status = "Excellent";
    else if (musclePercent >= 28) status = "Normal";
    else status = "Below Average";
  }
...
function assessWater(vitals, patient, history) {
  ...
  // Status based on gender — Watson equation outputs: M ~50-65%, F ~40-60%
  let status = null;
  if (sex === 1) { // Male
    if (waterPercent >= 50 && waterPercent <= 65) status = "Optimal";
    else if (waterPercent >= 45 && waterPercent < 50) status = "Low";
    else if (waterPercent > 65) status = "High";
    ...
    else if (waterPercent > 60) status = "High";
    else status = "Dehydrated";
  }
...
function assessMuscleControl(vitals, patient, history, scanCount) {
  ...
  const musclePercent = bodyComposition.calc_muscle_percent(weight, height, sex, age, impedance);
  const absoluteMuscleMass = bodyComposition.calc_muscle_mass(weight, musclePercent);
  const gap = bodyComposition.calc_muscle_control(weight, musclePercent, sex);
  const userName = getFirstName(patient);
  const isMale = sex === 1;
  ...
  return { status, gap, remedy, comment, absoluteMuscleMass };
}
...
function assessIdealBodyWeight(vitals, patient, history, scanCount) {
  ...
  const healthyRange = bodyComposition.calc_healthy_weight_range(height);
  ...
  return { status, ideal: idealWeight, remedy, comment, diff, healthyRange };
}
...
const Report2 = () => {
  ...
  const systemsData = useMemo(() => {
    ...
    // When multiple systems tie for the highest score, pick based on which
    // system has the best value relative to its ideal range (normalized score)
    const tiedSystems = Object.keys(scores).filter(key => scores[key] === maxScore);
    let strongestSystem = tiedSystems[0];
    if (tiedSystems.length > 1) {
      // Use assessment value distance from ideal to break ties
      const sex = patient.gender?.toLowerCase() === "male" ? 1 : 0;
      const assessmentMap = { BMI: bmi, "Body Fat": bodyFat, "Muscle Mass": muscle, "Bone Mass": boneMass, "Water Balance": water };
      const idealMidpoints = {
        BMI: { value: bmi.value, ideal: 22.0 },
        "Body Fat": { value: bodyFat.value, ideal: sex === 1 ? 18.0 : 26.0 },
        "Muscle Mass": { value: muscle.value, ideal: sex === 1 ? 45.0 : 35.0 },
        "Bone Mass": { value: boneMass.value, ideal: sex === 1 ? 3.0 : 2.2 },
        "Water Balance": { value: water.value, ideal: sex === 1 ? 57.0 : 52.0 }
      };
      // Closest to ideal wins the tie
      let bestDistance = Infinity;
      for (const sys of tiedSystems) {
        const info = idealMidpoints[sys];
        if (info && info.value != null) {
          const distance = Math.abs(info.value - info.ideal) / info.ideal;
              ...
              {scanCount >= 3 && muscleControl.status && (
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Skeletal Muscle:</span>
                      <span className="text-base font-semibold text-gray-800">
                        {muscleControl.absoluteMuscleMass?.toFixed(1)} kg
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-semibold text-gray-800">
                        {idealBodyWeight.healthyRange?.min.toFixed(1)} - {idealBodyWeight.healthyRange?.max.toFixed(1)} kg
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
