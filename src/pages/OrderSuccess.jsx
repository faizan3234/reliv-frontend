...
function DispensingAnimation({ cart = [], onComplete }) {
  ...
  const totalItems = safeCart.reduce((sum, item) => sum + (item.cartQuantity || 1), 0);

  useEffect(() => {
    if (totalItems === 0) { onComplete(); return; }
    // Simulate per-item progress (2s per item rotation on ESP32)
    ...
    // Auto-advance after all items + buffer
    const timer = setTimeout(onComplete, totalItems * 2200 + 2000);
    ...
    >
      ...
      <div className="relative z-10 bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl p-10 max-w-lg w-full border border-green-200">
        ...
        <p className="text-gray-600 mt-4 text-xl leading-relaxed">
          Your wellness kits are being dispensed right now.
          <br />Please collect them from the tray below.
        </p>

        {/* Per-kit progress */}
        <div className="mt-8 space-y-3">
          {safeCart.map((item) => (
            <div key={item.id} className="flex items-center gap-3 bg-green-50 rounded-xl px-5 py-3">
              <motion.span
                animate={{ scale: [1, 1.3, 1] }}
                transition={{ duration: 1.2, repeat: Infinity }}
                className="text-2xl"
              >
                {dispensed >= (item.cartQuantity || 1) ? '✅' : '⏳'}
              </motion.span>
        ...
        <div className="mt-8 bg-gray-200 rounded-full h-3 overflow-hidden">
          <motion.div
            ...
            initial={{ width: '0%' }}
            animate={{ width: totalItems > 0 ? `${Math.min((dispensed / totalItems) * 100, 100)}%` : '100%' }}
            transition={{ duration: 0.5 }}
          />
        </div>
        <p className="mt-3 text-sm font-medium text-green-700">
          {dispensed >= totalItems ? '🎉 All kits dispensed!' : `Dispensing ${dispensed} of ${totalItems}...`}
        </p>
      </div>
  ...
  return (
    <div className="relative min-h-screen">
      <AnimatePresence mode="wait">
        ...
        {step === 'payment' && <PaymentSuccess key="payment" onComplete={() => setStep(hasKits ? 'dispensing' : 'uv')} />}
