...
function assessLBMI(vitals, patient, scanCount) {
  ...
  const lbmi = bodyCompositionUtils.calc_lbmi(weight, height, age, impedance);
  const userName = getFirstName(patient);
  const isMale = sex === 1;
...
export default function Report3() {
  ...
  const normalizedScores = useMemo(() => {
    ...
    return {
      muscle: clamp((metrics.musclePct - (isMale ? 25 : 18)) / 25 * 100),
      fat: clamp((30 - metrics.bodyFatPct) / 20 * 100),
      water: clamp((metrics.waterPct - 45) / 25 * 100),
    };
  }, [metrics, isMale]);
  ...
  const insights = useMemo(() => {
    ...
    return list.length > 0 ? list : ["Your first scan provides a solid foundation—see the page evolve with new data and patterns in future scans."];
  }, [metrics, scanCount]);
  ...
  return (
    <div style={{ height: "100vh", background: "#ffffff", display: "flex", flexDirection: "column", alignItems: "center", padding: "48px 32px", overflowY: "auto", WebkitOverflowScrolling: "touch" }} className="scrollable-container">
      <div style={{ width: "100%", maxWidth: "1000px", margin: "0 auto" }}>
        ...
        {/* STRONGEST AREA - Unlocks after scan 4 */}
