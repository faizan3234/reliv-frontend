...
const BloodPressurePage = () => {
        ...
        try {
          if (topic === "kiosk/sensor/bp") {
            ...
            if (data.systolic && data.diastolic && data.bpm) {
              ...
              if (isAbnormalBP(data.systolic, data.diastolic, data.bpm)) {
  ...
  // Check if BP values are abnormal/unrealistic
  const isAbnormalBP = (systolic, diastolic, bpm) => {
    return (
      systolic > 180 || 
      diastolic > 120 || 
      bpm > 120 || 
    ...
    if (systolic && diastolic) {
      update({
        vitals: {
          ...
          systolic: systolic,
          diastolic: diastolic,
          bpm: bpm || data.vitals?.bpm, // Use BP bpm if available, else keep existing
        },
      });
